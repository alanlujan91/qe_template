...
Try to avoid excessive use of italics and bold face; underlining is generally banned (except for exceptional cases). Please do not use any $\LaTeX$ or $\TeX$ commands that affect the layout or formatting of your document (i.e., commands like \texttt{{\textbackslash}textheight}, \texttt{{\textbackslash}textwidth}, etc.). Note that the Introduction should be Section{\textasciitilde}$\ref{s1}$ it should not immediately follow the abstract without a heading.
...
instead of \verb|\newtheorem|.
\end{fact}
\begin{proof}[Proof of Theorem 2]This is the body of the proof of Theorem~\ref{th2}.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/figure_sample-dc8c28e403da531eb439ea35cd9b1b14.pdf}
...
\subsection{First subsection of Appendix B}\label{appB1}
...
Sample of cross-reference to formula (\ref{path}) in Appendix{\textasciitilde}$\ref{appB1}$.
Note that it is better to refer to Appendix{\textasciitilde}$\ref{appB1}$ as opposed to Appendix{\textasciitilde}$\ref{appB}$, because it is easier for the reader to locate the necessary place.
