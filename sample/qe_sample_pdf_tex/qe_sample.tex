...
\documentclass[qe,nameyear,final,openaccess]{econsocart}
%
%\usepackage{}
...
%% Preprint / neutral mode (journal: preprint): strip all journal identification
%% so the PDF does not signal which Econometric Society journal it targets. The
%% qe layout is used for formatting; the "Submitted to ..." banner, the journal
%% running head, and the copyright line are removed.
...
Try to avoid excessive use of italics and bold face; underlining is generally banned (except for exceptional cases). Please do not use any LaTeX2e or TeX commands that affect the layout or formatting of your document (i.e., commands like \texttt{{\textbackslash}textheight}, \texttt{{\textbackslash}textwidth}, etc.). Note that the Introduction should be Introduction it should not immediately follow the abstract without a heading.
...
\begin{enumerate}
\item This is the first item of an enumerated list. Each item in the list is marked with a ``tick.'' The document style determines what kind of tick mark is used.
\item This is the second item of the list. It contains another list nested inside of it.

\begin{enumerate}
\item This is the first item of an enumerated list that is nested within.
\item This is the second item of the inner list. LaTeX allows you to nest lists deeper than you really should.
\end{enumerate}
...
Parenthetical cite: \citep{b1}.
Multiple narrative cites: \citet{b2, b3, b4}.
...
\begin{itemize}
\item Regular text \texttt{{\textbackslash}textrm\{\}}
...
\end{proof}\begin{proposition}\label{cl1}This is the body of Proposition~\ref{cl1}.

\end{proposition}\begin{axiom}\label{ax1}This is the body of Axiom~\ref{ax1}. Axioms should be on a different counter from results (e.g. Theorems, Propositions, Lemmas).
...
Theorem~\ref{th2} because we used \texttt{[theorem]} in \texttt{{\textbackslash}newtheorem}.

\end{lemma}\begin{observation}This is the body of the fact. Fact is unnumbered because we used the command \texttt{{\textbackslash}newtheorem*}
instead of \texttt{{\textbackslash}newtheorem}.

\end{observation}\begin{proof}[Proof of Theorem 2]This is the body of the proof of Theorem~\ref{th2}.
...
\end{definition}\begin{example}This is the body of the example. Example is unnumbered because we used \texttt{{\textbackslash}newtheorem*}
instead of \texttt{{\textbackslash}newtheorem}.
...
\begin{figure}[!htbp]
\centering
\includegraphics[width=0.7\linewidth]{files/figure_sample-7bd7fd6fc01a4353f19ac08bae424317.pdf}
...
Appendices are provided through the \texttt{parts.appendix} frontmatter. If there is only one appendix, then please refer to it in text as ... in the~Appendix. If there is more than one appendix, then please refer to them as ... in Appendix~A, Appendix~B, etc. A bare \texttt{@appA} renders the heading title rather than the letter (a \href{https://github.com/executablebooks/mystmd/issues/1924}{known MyST limitation}), so type the letter in the link text.

\subsection{Title of the second appendix}\label{appB}

\subsubsection{First subsection of Appendix B}\label{appB1}

If your appendix is long, make sure to divide it into subsections and refer to them in text. Use the standard LaTeX commands for headings in a \texttt{\{appendix\}} environment. Headings and other objects will be numbered automatically.
...
Sample of cross-reference to formula (\ref{path}) in Appendix~B.1. Note that it is better to refer to Appendix~B.1 as opposed to Appendix~B, because it is easier for the reader to locate the necessary place.
